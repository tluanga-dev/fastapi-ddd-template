...
async def create_inventory_unit(
    ...
    use_case = CreateInventoryUnitUseCase(inventory_repo, stock_repo, sku_repo)
    
    try:
        unit = await use_case.execute(
            inventory_code=unit_data.inventory_code,
            sku_id=unit_data.sku_id,
...
async def create_stock_level(
    ...
    existing = await repo.get_by_sku_location(stock_data.sku_id, stock_data.location_id)
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Stock level already exists for SKU {stock_data.sku_id} at location {stock_data.location_id}"
        )
...
@router.get("/stock-levels/{sku_id}/{location_id}", response_model=StockLevelResponse)
async def get_stock_level(
    sku_id: UUID,
    location_id: UUID,
    db: AsyncSession = Depends(get_db)
) -> StockLevelResponse:
    """Get stock level for SKU at location."""
    repo = SQLAlchemyStockLevelRepository(db)
    stock_level = await repo.get_by_sku_location(sku_id, location_id)
    
    if not stock_level:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Stock level not found for SKU {sku_id} at location {location_id}"
        )
    
    return StockLevelResponse.model_validate(stock_level)


@router.put("/stock-levels/{sku_id}/{location_id}/operation", response_model=StockLevelResponse)
async def perform_stock_operation(
    ...
    use_case = UpdateStockLevelsUseCase(stock_repo, sku_repo, location_repo)
...
@router.put("/stock-levels/{sku_id}/{location_id}/parameters", response_model=StockLevelResponse)
...
@router.put("/stock-levels/{sku_id}/{location_id}/reconcile", response_model=StockLevelResponse)
...
async def check_stock_availability(
    ...
    use_case = CheckStockAvailabilityUseCase(inventory_repo, stock_repo, sku_repo, location_repo)
